...
- `ios-simulators`: one finding, `ios-simulators.devices`, from `xcrun simctl list -j devices` (10 s timeout, through the process port): a device is proposed when its runtime is gone (`isAvailable: false`) or its `lastBootedAt` is older than `unused-after`; a device that never booted and is still available is left alone. The path is `~/Library/Developer/CoreSimulator/Devices/<udid>`, sized from the walk. A failing or missing `xcrun` is a `location_unreadable` warning, never a failure; a home without CoreSimulator never runs `xcrun`.
