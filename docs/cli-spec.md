...
> This document supersedes draft 1.0 (`broza-cli-spec.md`, Spanish). All changes relative to 1.0 are listed in §9 "Changelog 1.0 → 1.1", §10 "Changelog 1.1 → 1.2", §11 "Changelog 1.2 → 1.3" and §12 "Changelog 1.3 → 1.4". Requirement identifiers (`RF-xx`, `RNF-xx`) refer to the Broza PRD. This specification is normative: "MUST", "MUST NOT", "SHOULD" and "MAY" are used in their RFC 2119 sense.
...
- `ios-simulators`: one finding, `ios-simulators.devices`, from `xcrun simctl list -j devices` (10 s timeout, through the process port): a device is proposed when its runtime is gone (`isAvailable: false`) or its `lastBootedAt` is older than `unused-after`; a device that never booted and is still available is left alone. The path is `~/Library/Developer/CoreSimulator/Devices/<udid>`, sized from the walk (falling back to the listing's `dataPathSize`, an apparent size and the only one the listing offers, when the walk did not reach it); a device measuring zero bytes is left out. A device whose `udid` is not a UUID, or whose availability the listing does not state, is ignored: the listing is another program's output and never names what Broza proposes. A failing or missing `xcrun` is a `location_unreadable` warning, never a failure; a home without CoreSimulator never runs `xcrun`.
...
- `duplicates`: candidates are grouped by size, then by their first and last 4 KiB, then confirmed by a full BLAKE3 hash. Only confirmed groups are reported.
- `duplicates`, scope: one finding, `duplicates.home`, over the regular files of at least 1 MB and under 1 GB (apparent size, as the Finder shows it; larger files are `large-old-files`' business, and hashing one costs seconds) the home walk reported, in the user's own trees only: nothing under a hidden directory (`.cache`, `.git`, `.vscode`, `.Trash`), under a `Library`, `node_modules`, `target`, `build` or `__pycache__` component at any depth, or inside a macOS package (`.app`, `.sparsebundle`, `.photoslibrary`, `.fcpbundle`, `.imovielibrary`, `.tvlibrary`, `.musiclibrary`, `.pvm`, `.utm`, `.xcarchive`, `.band`, `.rtfd`, `.pages`, `.numbers`, `.key`), whose identical files are structure, not copies. A file with more than one hard link is never a candidate: quarantining one name frees nothing. Neither is an APFS clone, nor a file that has a clone among the reported files: the family shares its blocks, and quarantining one member frees nothing while another stands. In each confirmed group the copy with the oldest modification time is kept (ties by path; a copy whose modification time is unknown is kept only when no other is) and the rest are proposed, one `paths[]` entry each; the `reasoning` counts the groups. Sizes and dates come from the walk, taken before any file is read; reading a file for comparison updates its access time. Contents are compared when `suggest` runs; `clean` re-checks each item's identity and size, not its contents, so a kept copy edited in between leaves the quarantined copy as the only one of its version, restorable from quarantine. The `reasoning` states the clone rule. A file that cannot be read for comparison is a `location_unreadable` warning and is left out of its group.
...
- `unused-apps`: three findings. `unused-apps.applications` (amber, `quarantine`): `*.app` bundles one folder deep under `/Applications` and `~/Applications` whose Spotlight `kMDItemLastUsedDate` (`mdls`, 5 s timeout, through the process port) is older than `unused-after`; that date is `paths[].last_used`. `unused-apps.unverified` (red, `inform_only`): the same for bundles Spotlight has no date for, judged by the bundle's own access and modification times and stated as low confidence; Broza lists them with `instructions` for checking and removing them by hand and never acts on them, so they count in `inform_only_bytes` and nowhere else. `unused-apps.leftovers` (amber, `quarantine`): directories directly under `~/Library/{Caches, Logs, HTTPStorages, Saved Application State}` named by a bundle identifier (three or more dot-separated labels, not `com.apple.*`; the `.savedState` suffix stripped) that no installed application carries — installed means the `CFBundleIdentifier` of a bundle under `/Applications`, `~/Applications` or `/System/Applications`, one folder deep, or of a helper, XPC service, extension or login item inside one, or any identifier extending an installed one with a further label — and whose own and children's modification times are all older than `unused-after`; sized from the home walk. `Application Support`, `Containers` and `Group Containers` are deliberately out: an installed program may still own them. A leftover under `Caches` that `user-cache` also reports is credited to `user-cache` (§3.3, overlap). Bundles under `/Applications` are sized by a walk of the bundle, candidates only, a hole inside being a warning; Apple's applications under `/System/Applications` are inventory, never proposed. An applications folder that cannot be read is a `location_unreadable` warning; a failing `mdls` is one `spotlight_unavailable` warning and every candidate is judged by its times.
- `large-old-files`: one finding, `large-old-files.home`, over the regular files of at least 1 GB (apparent size) the home walk reported, outside `~/Library` (application-managed; other detectors' ground) and `~/.Trash` (the `trash` detector's). Dates come from the walk, taken before any detector reads a file. A file is proposed when its modification time and its `last_used` are both older than `unused-after`. Spotlight is asked through `mdls -name kMDItemLastUsedDate -raw` (5 s timeout, through the process port) only about files whose modification and access times already qualify, and not at all once `mdls` has failed or timed out: that is one `spotlight_unavailable` warning, and the remaining files are judged by access time alone. A file with no modification time, with neither an access time nor a Spotlight date, a cloud placeholder, or a file with more than one hard link (quarantining one name frees nothing) is never proposed. When Spotlight has no date for some of the proposed files, the `reasoning` counts them and states low confidence.
- `cloud-synced`: always `risk: red`, `actionable: false`, `action: inform_only`. No flag changes this. One finding per provider present, `cloud-synced.<provider>` with `provider` one of `icloud`, `dropbox`, `onedrive`, `google-drive`, `other`. Roots: iCloud Drive at `~/Library/Mobile Documents`; every File Provider root under `~/Library/CloudStorage` (grouped by the entry's prefix: `Dropbox-…`, `OneDrive-…`, `GoogleDrive-…`; anything else is `other`); the legacy `~/Dropbox`, `~/OneDrive` and `~/Google Drive` when present. The home walk leaves these roots out (§7); the detector walks each root itself, never entering a directory the provider has not downloaded and counting an evicted file as 0 bytes. `reclaimable_bytes` is the **allocated** bytes on this disk, what the provider could release; `item_count` the local files; `paths[]` the roots; `instructions` the provider's official steps. A root that cannot be read is a `location_unreadable` warning; so is a hole inside one (one warning per root), and the figure is then a floor, which the `reasoning` states.
...
REVIEW (amber) — 96.1 GB
   duplicates        52.3 GB   Duplicate files (52.3 GB)
...
4. The path is under an allowed root: `$HOME`, `/Users/Shared`, `/private/var/folders/<uid>`, `/Library/Caches`, `.Trashes` on data or user volumes, `/Applications` (only for `unused-apps`) — and under none of the cloud-provider roots of §7, whatever detector named it and however it is spelled: a synced file deleted locally is deleted from the cloud and every other device (AGENTS.md §2.5), so the plan is rejected (exit `2`) before anything is looked at.
...
```
$ broza about
Broza 1.0.0  ·  MIT License  ·  JSON schema 1.1
...
| `file_report_truncated` | `warnings[]` | The home holds more files above the 1 MB reporting threshold than the walk keeps (200 000, the biggest); `duplicates` and `large-old-files` saw only those. |
...
Scanning is parallel per volume. The scan cache lives in `~/.cache/broza/v1/<volume_uuid>/`, one store per volume, keyed by `(dev, inode, mtime)` of each directory and expired by the configured `cache-ttl`. Each record carries the directory's aggregate, the names and keys of the directories directly inside it, the clone families whose credited clone is directly inside it, and every file directly inside it of at least 1 MB (name, sizes, inode, link count, clone id, times). A directory whose key is unchanged is served from the cache **whole**: the store rebuilds every directory below it and every file it kept, so the report shows the same directories and files a walk would have, and the subtree is not walked. A subtree is served only when every directory below it has a fresh record too, when it holds no hard link to a name outside it and no unreadable hole, and when the cache carries every file the request could list, which is to say when `--min-size` is 1 MB or more; below that the store is not read (so `--min-size 0` disables the reuse). A warm scan therefore reports exactly what a cold one would, up to the staleness the TTL bounds: a file that grows in place does not change its directory's mtime, and the record stands until `cache-ttl` or `--no-cache`. The same holds for a directory whose mtime a tool set back after changing its contents (`rsync -t`, `tar -p`, a restore): the record's names are served, so a warm report can list entries that are gone or miss new ones until the TTL. The store carries a versioned magic header; a file written by an older Broza is discarded and rewritten, any other decode error is exit `9` (`CACHE_ERROR`) with the hint to retry with `--no-cache`, and Broza never attempts to "repair" a corrupt cache silently. `--no-cache` bypasses reads but still writes a fresh cache. A volume with no UUID falls back to its BSD name, with a `cache_keyed_by_bsd_id` warning ([ADR 0008](adr/0008-cache-records-carry-their-children.md)).
...
The walk lists every directory through a descriptor (`getattrlistbulk` on the descriptor, `fstatat` for what it leaves out) and reaches a directory deeper than a path can name — macOS refuses paths of 1024 bytes or more — by relative steps from the deepest ancestor that opens, so such a tree is measured to the bottom ([ADR 0010](adr/0010-walk-by-directory-descriptor.md)); items below that depth appear in the report and can never be plan items, because every write is a path call the kernel refuses.
...
- **Partial-clone accounting** (the blocks a diverged clone owns), HFS+ specifics beyond enumeration, FAT/exFAT support.
...
- §3.3 (M5): `unused-apps` detector — `applications` (amber), `unverified` (red, inform only, low confidence), `leftovers` (amber, by bundle identifier, regenerable locations only).
- §3.4 and §7 (M5): the cloud-provider roots are five (`~/Library/Mobile Documents`, `~/Library/CloudStorage`, `~/Dropbox`, `~/OneDrive`, `~/Google Drive`), excluded from the home walk and refused by the safety kernel for every plan item (`InsideCloudRoot`).
- §3.3 (M5): `cloud-synced` reports a hole inside a root as a warning and states its figure is a floor; `unused-apps` warns about a bundle whose manifest it cannot read and judges a leftover by everything directly inside it.
...
- §5 (post-1.0): the message is set apart by blank lines and a `─` rule above and below; the first line is bold and the rest dim where color is enabled. Still no color.
