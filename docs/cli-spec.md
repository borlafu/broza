...
- `unused-apps`: three findings. `unused-apps.applications` (amber, `quarantine`): `*.app` bundles one folder deep under `/Applications` and `~/Applications` whose Spotlight `kMDItemLastUsedDate` (`mdls`, 5 s timeout, through the process port) is older than `unused-after`; that date is `paths[].last_used`. `unused-apps.unverified` (red, `quarantine`): the same for bundles Spotlight has no date for, judged by the bundle's own access and modification times and stated as low confidence; red, so `clean --apply` refuses them and the list is for the user. `unused-apps.leftovers` (amber, `quarantine`): directories directly under `~/Library/{Application Support, Caches, Containers, Saved Application State, Logs, HTTPStorages, WebKit}` named by a bundle identifier (three or more dot-separated labels, not `com.apple.*`; the `.savedState` suffix stripped) that no installed application carries — installed means the `CFBundleIdentifier` of a bundle under `/Applications`, `~/Applications` or `/System/Applications`, one folder deep — and untouched for `unused-after` by the directory's mtime; sized from the home walk. Bundles under `/Applications` are sized by a measurement, candidates only; Apple's applications under `/System/Applications` are inventory, never proposed. An applications folder that cannot be read is a `location_unreadable` warning; a failing `mdls` is one `spotlight_unavailable` warning and every candidate is judged by its times.
