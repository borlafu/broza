...
- [x] Deep trees (post-1.0, ADR 0010): the walk lists each directory through a descriptor, reaches
      one deeper than a path can name by relative steps from the deepest ancestor that opens, and
      states the bulk reader's leftovers with `fstatat`, so `PATH_MAX` no longer ends a scan
      subtree; `tests/deep_paths.rs` walks 520 levels on the real filesystem, under `ulimit -n 256` too.
